System.Management.Automation.DefaultParameterDictionary = @{ 'Invoke-WebRequest:UseBasicParsing' = $true }
...
function Invoke-ApiJson {
  ...
  try {
    if ($null -ne $bodyText) {
      $resp = Invoke-WebRequest -Method $Method -Uri $Url -ContentType "application/json" -Body $bodyText
    } else {
...
if (-not [string]::IsNullOrWhiteSpace($assignmentId)) {
  ...
  $uploadOut = curl.exe -sS -X POST "$base/api/v1/submissions/$assignmentId/upload?student_id=stu-smoke-1&student_name=Smoke+Student" -F "file=@$uploadFile"
  $uploadJson = $null
  try { $uploadJson = $uploadOut | ConvertFrom-Json } catch { }
  if ($null -ne $uploadJson) {
    Add-Result "submissions.upload" $true 202 ($uploadOut.Substring(0, [Math]::Min(240, $uploadOut.Length)))
  } else {
    Add-Result "submissions.upload" $false 0 ($uploadOut.Substring(0, [Math]::Min(240, $uploadOut.Length)))
  }
  ...
  if (-not [string]::IsNullOrWhiteSpace($codeAssignmentId)) {
    ...
    # jobs list (create is contract-heavy and environment-ready dependent)
    Invoke-ApiJson -Name "code_eval.jobs.list" -Method "GET" -Url "$base/api/v1/code-eval/jobs" -Allowed @(200) | Out-Null
  }

  # optional delete cleanup (allowed to fail due FK constraints)
  Invoke-ApiJson -Name "assignments.delete" -Method "DELETE" -Url "$base/api/v1/assignments/$assignmentId" -Allowed @(204,409,500) | Out-Null
}
